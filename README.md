...
本リポジトリは**MacOS**，**Python 3.12**での動作を確認しています．
...
- データの前処理用ファイル **src/preprocessor/preprocess.ipynb** を全て実行しデータの前処理を行ってください．
  ...
  - 前処理の実行には映像データへの加工が含まれるため，1時間以上かかる場合があります．
...
**初回動作時は１時間近くかかる可能性があります（映像データの前処理があるため）．**
...
- basketball_court.mp4：ボールコート上のボロノイ領域の映像
